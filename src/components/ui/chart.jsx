...
const ChartLegendContent = React.forwardRef(({
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "flex flex-col gap-1 px-2 py-1 text-xs text-muted-foreground",
        verticalAlign === "top" && "border-b",
        verticalAlign === "bottom" && "border-t",
        className
      )}
    >
      {payload.map((entry, i) => {
        const key = `${nameKey || entry.value || entry.dataKey || "value"}`
